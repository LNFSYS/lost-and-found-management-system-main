...
export function RegisterPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setError(""); setPending(true); try { await api.register(form); await refreshUser(); navigate("/profile", { replace: true }); } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể tạo tài khoản"); } finally { setPending(false); } }
