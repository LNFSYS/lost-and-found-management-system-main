...
import { authService } from "../../../test/use-case-fixtures.js";
...
const { requireAnyRole, requireAuth } = createAuthMiddleware({ authService, verifyAccessToken: (token) => jwt.verify(token, env.jwtAccessSecret) as Parameters<typeof authService.validateAccessSession>[0] });
