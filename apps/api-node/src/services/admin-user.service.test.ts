...
function fakeRepository(seed: AdminUserRecord[] = []) {
  ...
  return { repository, users, passwordHashes, revokedRefreshTokenUserIds };
}

function serviceFor(repository: AdminUserRepository) {
...
test("admin user service supports CRUD without leaking sensitive fields", async () => {
  ...
  const { repository, passwordHashes, revokedRefreshTokenUserIds } = fakeRepository([admin]);
  ...
  const updated = await service.updateUser(created.id, { fullName: "Updated Member", phoneNumber: "0909000000" });
...
test("admin user service rejects duplicate email", async () => {
  ...
  await assert.rejects(() => service.updateUser(user.id, { email: "admin@example.com" }), (error: unknown) => (
    error instanceof HttpError && error.status === 409
  ));
});
