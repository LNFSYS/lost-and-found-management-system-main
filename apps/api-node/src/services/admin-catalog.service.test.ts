...
import { adminCatalogRepository, type AdminHandoverPoint } from "../repositories/admin-catalog.repository.js";
...
test("hard delete is rejected while an active appointment uses the handover point", async () => {
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => error instanceof HttpError && error.status === 409 && /lịch hẹn hoạt động/.test(error.message)
    );
