...
import { matchingRepository, postRepository } from "../../../test/persistence-fixtures.js";
import { postReadScope } from "../domain/post-access.js";
import { buildListWhere } from "./post.repository.js";
...
test("board SQL excludes other users' private posts while retaining the viewer's own posts", () => {
  ...
  const regular = buildListWhere(filters, undefined, postReadScope(viewer));
  const anonymous = buildListWhere(filters);
  const staff = buildListWhere(filters, undefined, postReadScope({ ...viewer, roles: ["USER", "STAFF"] }));
