import type { MediaKind, PostMediaRecord, PostRecord, PostRepository, PostStatus, PostType, StoredMediaRecord, VisibilityMode } from "../application/post.repository.port.js";

export type { MediaKind, PostMediaRecord, PostRecord, PostRepository, PostStatus, PostType, StoredMediaRecord, VisibilityMode } from "../application/post.repository.port.js";
...
import type { ListOwnPostsQuery, ListPostsQuery } from "../application/post.dto.js";
...
export function buildListWhere(filters: ListPostsQuery | ListOwnPostsQuery, ownerId?: string, scope: PostReadScope = { allPrivate: false }) {
  ...
  if (ownerId) {
    ...
    if (ownerId || scope.allPrivate) {
      // The application has authorized unrestricted private-row filtering.
    } else if (scope.privateOwnerId) {
      where.push("(p.visibility_mode = 'PUBLIC' OR p.user_id = ?)");
      values.push(scope.privateOwnerId);
    } else {
  ...
  if (filters.buildingId) {
    if (ownerId || scope.allPrivate) {
      ...
      values.push(filters.buildingId, scope.privateOwnerId, filters.buildingId);
    } else {
  ...
  if (filters.q) {
    ...
    if (ownerId || scope.allPrivate) {
      ...
      values.push(q, q, scope.privateOwnerId, q, q);
    } else {
...
export function createPostRepository(pool: SqlExecutor) {
  ...
  async function loadMedia(postIds: string[], queryable: Queryable = pool) {
    ...
    const [rows] = await sqlExecutor(queryable).execute<MediaRow[]>(
  ...
  async function listPosts(filters: ListPostsQuery | ListOwnPostsQuery, ownerId?: string, scope?: PostReadScope) {
    const { sql, values } = buildListWhere(filters, ownerId, scope);
  ...
  async function findPost(where: string, values: SqlValue[], queryable: Queryable = pool, lock = false) {
    const [rows] = await sqlExecutor(queryable).execute<PostRow[]>(`${postSelect} WHERE ${where} LIMIT 1${lock ? " FOR UPDATE" : ""}`, values);
    const row = rows[0];
    if (!row) return null;
  ...
  const postRepository = {
    ...
    listBoard(filters: ListPostsQuery, scope: PostReadScope) {
      return listPosts(filters, undefined, scope);
    },
    ...
    async lockOwnedPostForMedia(postId: string, ownerId: string, queryable: Queryable) {
      const [rows] = await sqlExecutor(queryable).execute<LockedPostRow[]>(
    ...
    async countMedia(postId: string, queryable: Queryable = pool) {
      const [rows] = await sqlExecutor(queryable).execute<CountRow[]>("SELECT COUNT(*) AS total FROM post_media WHERE post_id = ?", [postId]);
    ...
    async findMedia(postId: string, mediaId: string, queryable: Queryable = pool, forUpdate = false): Promise<StoredMediaRecord | null> {
      const [rows] = await sqlExecutor(queryable).execute<StoredMediaRow[]>(
    ...
    async deleteMedia(postId: string, mediaId: string, queryable: Queryable = pool) {
      const [result] = await sqlExecutor(queryable).execute<ResultSetHeader>("DELETE FROM post_media WHERE id = ? AND post_id = ?", [mediaId, postId]);
      return result.affectedRows > 0;
    },
