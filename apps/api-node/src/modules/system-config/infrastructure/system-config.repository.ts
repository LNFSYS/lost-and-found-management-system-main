import type { SystemConfigRecord, SystemConfigRepository } from "../application/system-config.repository.port.js";

export type { SystemConfigRecord, SystemConfigRepository } from "../application/system-config.repository.port.js";
...
import type { ConfigValueType, ListConfigsQuery } from "../application/system-config.dto.js";
...
export function createSystemConfigRepository(pool: SqlExecutor) {
  ...
  const systemConfigRepository = {
    ...
    async findById(configId: string, connection: Queryable = pool, forUpdate = false) {
      const [rows] = await sqlExecutor(connection).execute<ConfigRow[]>(
        `SELECT id, config_key, config_value, value_type, description, is_public, updated_by, updated_at
    ...
    async updateConfig(configId: string, input: {
      ...
      await sqlExecutor(connection).execute(`UPDATE config_entries SET ${fields.join(", ")} WHERE id = ?`, [...values, configId]);
    },

    async deleteConfig(configId: string, connection: Queryable) {
      const [result] = await sqlExecutor(connection).execute<ResultSetHeader>("DELETE FROM config_entries WHERE id = ?", [configId]);
    ...
    async listHistory(configId: string, configKey: string, limit: number, connection: Queryable = pool) {
      const [rows] = await sqlExecutor(connection).execute<ConfigHistoryRow[]>(
  ...
  } satisfies SystemConfigRepository;
