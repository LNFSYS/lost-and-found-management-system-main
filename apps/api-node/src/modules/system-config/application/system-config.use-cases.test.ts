...
import { createTestSystemConfigUseCases as createSystemConfigService } from "../../../test/use-case-fixtures.js";
...
function fakeRepository(seed: SystemConfigRecord[] = []) {
  ...
  const history: Array<{ configKey: string; oldValue: string | null; newValue: string; actorId: string; action: string; reason: string | null; }> = [];
  const repository: SystemConfigRepository = {
    ...
    async listHistory() {
      return history.map((entry, index) => ({
        ...
        oldConfigKey: entry.configKey,
        newConfigKey: entry.configKey,
...
function serviceFor(repository: SystemConfigRepository) {
  ...
  return createSystemConfigService({
    ...
    transaction: async (work) => work({} as TransactionContext),
    idFactory: () => `id-${++index}`
  });
}
...
test("system config service keeps credential-like keys out of the database", async () => {
  const service = serviceFor(fakeRepository().repository);
  await assert.rejects(() => service.createConfig("admin-id", {
    configKey: "security.jwt_secret",
    configValue: "server-secret",
    valueType: "STRING",
    isPublic: false
  }), (error: unknown) => error instanceof AppError && error.code === "invalid_input");
});

test("system config service rejects SMTP passwords and nested credential keys", async () => {
  const service = serviceFor(fakeRepository().repository);
  for (const configKey of ["smtp.pass", "google.application_credentials"]) {
    await assert.rejects(() => service.createConfig("admin-id", {
      configKey,
      configValue: "server-secret",
      valueType: "STRING",
...
test("system config service exposes history for an existing config", async () => {
  const { repository, history } = fakeRepository();
  const service = serviceFor(repository);
  const created = await service.createConfig("admin-id", {
    configKey: "client.max_title_length",
  ...
  const result = await service.listHistory(created.id, { limit: 20 });

  assert.equal(result.items.length, 1);
  assert.equal(history[0]?.action, "CREATE");
  assert.equal(history[0]?.reason, "Initial setup");
});
