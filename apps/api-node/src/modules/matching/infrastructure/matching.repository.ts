...
async function upsertResult(connection: PoolConnection | TransactionContext, source: MatchCandidate, match: ScoredMatch) {
...
export function createMatchingRepository(pool: SqlExecutor, withTransaction: SqlTransactionRunner) {
  ...
  const matchingRepository = {
    ...
    async replaceAnalysisTags(postId: string, input: {
      ...
      visibleText: string[];
      confidence: number;
    }, transaction?: PoolConnection | TransactionContext) {
      ...
      const replace = async (connection: PoolConnection | TransactionContext) => {
        await sqlExecutor(connection).execute(
