...
import { createTestReturnFeedbackUseCases as createReturnFeedbackService } from "../../../test/use-case-fixtures.js";
import { returnFeedbackSchema } from "../interfaces/http/return-feedback.validator.js";
import type { ReputationScoreRecord, ReturnAppointmentForFeedback, ReturnFeedbackRecord, ReturnFeedbackRepository } from "./return-feedback.repository.port.js";
...
test("non-participants cannot read eligibility or submit feedback", async () => {
  ...
  await assert.rejects(service.getEligibility(appointmentId, stranger), (error: unknown) => error instanceof Error && "code" in error && error.code === "forbidden");
  await assert.rejects(service.submitFeedback(appointmentId, { rating: 5, comment: null, idempotencyKey: "stranger-key" }, stranger), (error: unknown) => error instanceof Error && "code" in error && error.code === "forbidden");
});
