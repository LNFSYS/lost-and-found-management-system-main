...
export interface OtpRow extends RowDataPacket { id: string; otp_hash: string; attempt_count: number; max_attempts: number; expires_at: Date; consumed_at: Date | null; }
export interface ResetRow extends RowDataPacket { id: string; token_hash: string; attempt_count: number; max_attempts: number; expires_at: Date; consumed_at: Date | null; }
export interface RefreshRow extends RowDataPacket { id: string; user_id: string; expires_at: Date; revoked_at: Date | null; }
...
export const authRepository = {
  ...
  async findRefreshForUpdate(tokenHash: string, connection: Queryable) {
    const [rows] = await connection.execute<RefreshRow[]>("SELECT id, user_id, expires_at, revoked_at FROM refresh_tokens WHERE token_hash = ? LIMIT 1 FOR UPDATE", [tokenHash]);
    return rows[0] ?? null;
  },
  async revokeRefresh(id: string, connection: Queryable, replacementId?: string) {
    await connection.execute("UPDATE refresh_tokens SET revoked_at = UTC_TIMESTAMP(), replaced_by_token_id = ? WHERE id = ? AND revoked_at IS NULL", [replacementId ?? null, id]);
  },
