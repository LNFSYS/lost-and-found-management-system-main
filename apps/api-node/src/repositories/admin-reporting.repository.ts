...
export function createAdminReportingRepository(database: Queryable = pool) {
  return {
    ...
    async findReportById(reportId: string, connection: Queryable = database) {
      const [rows] = await connection.execute<ReportRow[]>(`${reportSelect} WHERE r.id = ? LIMIT 1`, [reportId]);
    ...
    async getDailyTrends(window: ReportingWindow): Promise<DailyTrendRow[]> {
      ...
      const rows = await Promise.all(queries.map(async (query) => {
        const [result] = await database.execute<DailyCountRow[]>(query.sql, query.values);
    ...
    async getStatusBreakdown(window: ReportingWindow): Promise<DashboardBreakdown> {
      async function query(sql: string, values: SqlValue[] = []) {
        const [rows] = await database.execute<StatusCountRow[]>(sql, values);
...
export const adminReportingRepository = createAdminReportingRepository();
export type AdminReportingRepository = ReturnType<typeof createAdminReportingRepository>;
