...
import type { ActivitySummary } from "../types/auth.js";
...
export const returnFeedbackRepository = {
  ...
  async listFeedbackForAppointment(appointmentId: string, connection: Queryable = pool) {
    const [rows] = await connection.execute<FeedbackRow[]>(`${feedbackSelect} WHERE rf.appointment_id = ? ORDER BY rf.created_at DESC`, [appointmentId]);
    return rows.map(mapFeedback);
  },

  async findFeedbackByReviewer(appointmentId: string, reviewerId: string, connection: Queryable = pool) {
    const [rows] = await connection.execute<FeedbackRow[]>(`${feedbackSelect} WHERE rf.appointment_id = ? AND rf.reviewer_id = ? LIMIT 1`, [appointmentId, reviewerId]);
  ...
  async findFeedbackById(feedbackId: string, connection: Queryable = pool) {
    const [rows] = await connection.execute<FeedbackRow[]>(`${feedbackSelect} WHERE rf.id = ? LIMIT 1`, [feedbackId]);
    return rows[0] ? mapFeedback(rows[0]) : null;
  },

  async getReputationForUpdate(userId: string, connection: Queryable = pool): Promise<ReputationScoreRecord> {
    const [rows] = await connection.execute<ReputationRow[]>(
...
export type ReturnFeedbackRepository = typeof returnFeedbackRepository;
