...
export const adminUserRepository = {
  async listUsers(filters: ListAdminUsersQuery) {
    ...
    const [countRows] = await pool.execute<CountRow[]>(`SELECT COUNT(*) AS total FROM users u ${sql}`, values);
  ...
  async findById(userId: string, connection: Queryable = pool) {
    const [rows] = await connection.execute<AdminUserRow[]>(`${selectAdminUser} WHERE u.id = ? GROUP BY u.id`, [userId]);
    return rows[0] ? mapAdminUser(rows[0]) : null;
  },

  async findByNormalizedEmail(normalizedEmail: string, exceptUserId?: string, connection: Queryable = pool): Promise<string | null> {
  ...
  async countActiveAdmins(connection: Queryable = pool) {
...
export type AdminUserRepository = typeof adminUserRepository;
