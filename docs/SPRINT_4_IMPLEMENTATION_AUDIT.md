...
| LNFS-51 | To Do, P1, assignee field Truong Quang Dat; description ghi PHAM NGUYEN ANH KHOA | **Implemented runtime; blocked for real data by LNFS-54** | return-feedback route/controller/service/repository/validator, migration 043, profile feedback UI, API/UI tests. | Feedback chỉ nhận participant của return đã COMPLETED và có dual confirmation hoặc custody outcome; idempotency chống duplicate; reputation log/score được ghi transaction; profile activity không trả evidence/chat/contact. Chưa có LNFS-54 runtime để tạo completed dual-confirmed return từ UI thật. | Gắn branch, commit, test evidence và PR; link dependency LNFS-54 trước khi move Done theo Jira. |
| LNFS-52 | To Do, P0, assignee PHAM NGUYEN ANH KHOA | **Partial current runtime** | claim.routes.ts/controller/service/repository/validator; claims-page.tsx; claim service/repository/privacy tests. | Claim request/decision/withdraw, participant authorization, private text room, evidence proxy và idempotency có. Guided questions, appointment, full return journey và isolated MySQL concurrency chưa có. | Giữ Partial; chạy DB integration và hoàn thiện guided verification/appointment trước release. |
| LNFS-53 | To Do, P0, assignee Tran The Luong | **Implemented locally; Jira/PR/manual QA pending** | claim verification routes/controller/validator/use-cases/repository; verification-question-templates.ts; claim-verification-panel.tsx; claim-verification use-case/domain/validator tests; claims-resilience.spec.ts; `docs/LNFS-53-GUIDED-VERIFICATION.md`. | Category-versioned questions, per-claim assignment isolation, hashed/discarded secret answers, explicit Finder decision, canonical `ACCEPTED` eligibility, correction audit, rate limit và idempotency đã có. Isolated MySQL suite chưa chạy trong lần evidence này. | Chạy manual three-user privacy QA và isolated MySQL integration; sau đó gắn reviewed PR/commit/Jira evidence trước khi move Done. |
...
| LNFS-58 | To Do, P1, assignee Truong Quang Dat | **Partial current runtime** | claim routes/services/repositories; claims-page.tsx; claim resilience E2E; notification service/UI. | Private REST text room, retry idempotency, cursor history, participant guard và claim notification có. Socket.IO, image chat, seen/unread realtime, report/block và meetup events chưa có. | Giữ Partial; tách realtime/image/notification nâng cao thành story nhỏ. |
...
| LNFS-60 | To Do, P1, assignee PHAM NGUYEN ANH KHOA | **Implemented core; UI/device QA pending** | profile-page.tsx; auth-context.tsx; auth.service.ts; cloudinary-avatar-storage.ts; manifest.webmanifest; sw.js; PWA QA doc. | Profile/activity/reputation, Cloudinary authenticated avatar flow, live upload/signed delivery/cleanup smoke test, manifest, service worker và offline shell đã có. Session fixture đã tương thích avatar; device/installability evidence còn thiếu. | Chạy manual Cloudinary profile UI/PWA QA, gắn screenshots và xác nhận media bài đăng local storage gap riêng. |
...
| LNFS-62 | To Do, P1, assignee Võ Chiêu Quân | **Partial integration/release scope** | Web PWA shell, claims-page.tsx, notification-center.tsx, Playwright PWA/post tests và CI browser job. | Claim/private text/evidence/claim notification đã có một phần; appointment, realtime, offline transaction và installability/device evidence chưa đủ. | Giữ như integration/release checklist, không đánh dấu Done trước khi dependency và manual evidence hoàn tất. |
| LNFS-63 | To Do, P0, assignee Võ Chiêu Quân | **Blocked / not ready** | Existing Playwright tests pass baseline và có guided Finder decision trong private room ở `claims-resilience.spec.ts`. | P2P E2E hiện dừng ở guided verification; chưa có meetup, dual handover, custody alternative end-to-end, no-show, dispute hoặc PWA installability/offline evidence. Phụ thuộc LNFS-54 đến LNFS-62. | Để cuối sprint dưới dạng release gate. Không đánh dấu Done bằng guided verification test riêng lẻ. |
...
- LNFS-60 có profile/reputation/activity baseline; LNFS-51 bổ sung write flow feedback + reputation event sau completed return.
...
Các command dưới đây được chạy trên repository fptu-lost-found-system-main trong đợt remediation ngày 06/09/2026; source đã được sửa có kiểm soát trên branch audit/fix.

| Command | Kết quả |
| --- | --- |
| npm --workspace @lnfs/api-node run test | **PASS**: 137 API test pass, 1 DB integration test skip an toàn |
| npm --workspace @lnfs/web run lint | **PASS**: Web TypeScript check |
| npm run build | **PASS**: API TypeScript build và Web Vite production build |
| npm --workspace @lnfs/web run e2e:home | **PASS 23/23**: auth resilience, post creation, matching view, claim-room stale response, mobile layout, Staff warehouse và Admin handover/map |
| npm run build:java | **BLOCKED**: máy không có Maven trong PATH; chưa kết luận Java source lỗi |
| npm run migrate | **Không chạy** để tránh tác động Aiven/shared DB; migration 046 là forward corrective migration và cần apply sau review |
| git diff --check | **PASS** |
| Markdown relative-link scan | **PASS**: link tương đối trong README/docs resolve được |

Các test hiện tại chưa bao phủ MySQL integration thật cho claim concurrency/migration, appointment, dual handover, realtime chat, PWA installability/offline device matrix và native mobile; unit/browser coverage cho claim privacy, pagination, retry và stale room đã được bổ sung.
...
| Node.js API | **Core/write owner** | Auth, posts, catalog, matching, warehouse, Gemini, admin user/config, claim/private text/evidence và claim notification route/service hiện có |
...
**Sprint 4 backlog vẫn chưa nên đánh dấu Done hàng loạt.** LNFS-52/LNFS-58 hiện là Partial vì đã có runtime một phần nhưng chưa đủ end-to-end; LNFS-54, guided verification, realtime, PWA installability và Native Mobile vẫn là gap. Cần tách ticket lớn, chạy DB integration riêng và gắn PR/Jira evidence khi có connector/quyền. LNFS-63 chỉ được dùng làm release gate sau khi dependency thực sự có runtime và test.
