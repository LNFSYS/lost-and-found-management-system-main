...
| LNFS-60 | To Do, P1, assignee PHAM NGUYEN ANH KHOA | **Partial** | apps/web/src/main.tsx; route-guard.tsx; context/auth-context.tsx; auth pages; profile-page.tsx; responsive E2E. | Auth/refresh/profile cơ bản có. Profile/activity/avatar vẫn thuộc scope riêng; PWA foundation được triển khai ở LNFS-61. Mobile phải hiểu là mobile-browser/PWA, không phải native mobile. | Hoàn tất profile/activity và bổ sung installability/device matrix cùng privacy-safe offline acceptance criteria. |
| LNFS-61 | To Do, P1, assignee field Truong Quang Dat; description ghi PHAM NGUYEN ANH KHOA | **Implemented for board/post PWA scope** | apps/web/src/pages/posts-page.tsx; post-detail-page.tsx; story-post-form.tsx; pwa.ts; public/manifest.webmanifest; public/sw.js; posts-page.spec.ts; story-post-form.spec.ts; pwa-posts.spec.ts | Responsive board, detail, LOST/FOUND management, multi-image input, manifest, service worker app shell và offline unsent guard có. Notification/background retry không thuộc scope này. | Gắn branch, commit, test evidence và PR vào Jira rồi chuyển review. Không đổi Jira assignee bằng text cũ. |
...
- PWA phải tách khỏi Native Mobile. Repository hiện có Web responsive, file input, manifest, service worker app shell và offline unsent guard cho board/post flow; notification/background retry vẫn là scope riêng.
...
| PWA | **Partial** | Responsive/mobile-browser board/post flow, manifest, service worker, application shell và offline unsent guard đã có; installability/device matrix và background retry chưa có evidence |
